...
# - Checks if term[index] is an atomic variable which sits in a correct
# context.
# - We have used the fact that if x is a variable, then it occurs in
# one of the following contexts:
    ...
    # --> '(x var)'
    # --> '(var x)'
# - So we check that is_owercase(x), and that one of the above cases hold.
...
# - Assumes index is in range of term (but could be made to sometimes works 
# even if it wasn't)
...
# with index either 0 or len(term) - 1 (and nor should it).
def is_var(term, index):
    if index > -1 and index < len(term):
        ...
        or (term[index - 1: index + 1] == '(%s' % (term[index]) \
            and term[index + 2] == ')'\
            and is_owercase(term[index + 1])) \
        or (term[index - 2] == '(' \
            and term[index:index + 2] == '%s)' % term[index] \
            and is_owercase(term[index - 1]))):
            return True
        else:
            return False
...
# --> Strings inside balanced brackets.
...
def is_app(string):
    ...
    if len(index_list) == 4:
        if ord(string[0]) == 40 \
        and ord(string[-1]) == 41 \
        and is_ss(string[1:index_list[2] - 1]) \
        and is_ss(string[index_list[1] + 1:-1]) \
        and string[index_list[1]:index_list[2]] == ' ':
            return True
        else:
            return False
    else:
...
def is_promotion(string):
    ...
    and string[index_list[len(index_list) - 3]: \
        ...
        if len(index_list) == 6:
            if is_ss(string[9:index_list[2] - 5])\
            and is_ss(string[index_list[1] + 5:index_list[4] - 4]) \
            and is_ss(string[index_list[3] + 4:-1]):
                return True
            else:
                return False
        else:
            if is_ss(string[index_list[afr_for - 3] + 2: \
                    index_list[afr_for] - 5]) \
            and is_ss(string[index_list[afr_for - 1] + 5: \
                    index_list[afr_for + 2] - 2]) \
            and is_ss(string[9:index_list[2] - 2]) \
            and is_ss(string[index_list[len(index_list) - 3] + 4:-1]) \
            and is_ss(string[index_list[len(index_list) - 5] + 2: \
                    index_list[len(index_list) - 2] - 4]):
                check_number1 = 0
                check_number2 = 0
                for x in range(1,afr_for - 5 + 1,2):
                    if is_ss(string[index_list[x] + 2:index_list[x + 3] - 2]):
                        check_number1 += 1
                for x in range(afr_for + 1, len(index_list) - 7 + 1,2):
                    if is_ss(string[index_list[x] + 2:index_list[x + 3] - 2]):
                        check_number2 += 1
                if check_number1 == afr_for/2 - 2 \

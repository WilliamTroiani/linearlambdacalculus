...
def term_finder(term): # assuming we have a valid term
    ...
    if len(term) == 1:
        ...
        for x in term:
            if is_var2(x, index1):
                inside_terms.append(index1)
                inside_terms.append(index1 + 1)
                return inside_terms
            elif ord(x) == 40:
                inside_terms.append(index1)
                inside_terms.append(bracket_counter(term))
                inside_terms.append(term_finder(\
                        term[bracket_counter(term) + 1:-1]))
                return inside_terms
                break
        index1 += 1
